...
class VLM:
    ...
    def _format_allowed_cites(self, chunks, used_visuals) -> List[str]:
        ...
        # A multi-sheet figure contributes several images but is ONE citable
        # entity — dedupe so the whitelist doesn't repeat it per sheet.
        seen_cites = set()
        for kind, v in used_visuals:
            if kind == "Figure":
                line = f"{v.get('figure_id','?')} (p.{v.get('page_printed','?')})"
            else:
                line = f"Page {v.get('page_printed','?')}"
            if line not in seen_cites:
                seen_cites.add(line)
                allowed_cites.append(line)
