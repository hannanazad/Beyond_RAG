...
```
  ...
  │ 8,240 nodes, 18,245 edges│      with `contains`, `cites_*`, `defines`, `depicts`,
  ...
  ─────────────────────  query-time  ─────────────────────────────────────────
   ...
   query
     ...
     │
     ├─ scoring: S = λ1·S_RRF + λ2·S_graph + λ3·(w(rule_type) − 1) + λ4·S_hier
     │            (S_RRF is the ALREADY-FUSED rank score — there is no
     │             separate dense or sparse term in the implementation)
     │
...
\[
S(q, c_i) \;=\;
\lambda_1 \cdot S_{\text{RRF}}(q, c_i) \;+\;
...
| **Lightweight KG (NetworkX, no Neo4j)** | 8,240 nodes / 18,245 edges fits in a few MB pickle. Built deterministically from the PDF and regex extraction — no LLM in the ingestion loop, so no hallucinated edges. Same expressive power as Neo4j for our size and use case. |
...
## Corrections (see CHANGES.md)

Earlier revisions of this document published
`S = a*dense + b*sparse + c*hierarchy + d*graph + e*rule_type`, with separate
dense and sparse terms. The implementation has neither: `base` in
`retrieval.py` is the already-fused RRF score, and `w_sparse` in `config.py`
was never referenced by any code path. The formula above matches the code.

The graph-expansion step in `retrieval.py` is a placeholder. Only proximity
re-scoring of the fused candidate set happens; no new candidates are pulled in.

The sparse leg of BGE-M3 has not produced a signal in practice —
`chunks_sparse.json` is empty for every chunk, so RRF runs on the dense
ranking alone.
